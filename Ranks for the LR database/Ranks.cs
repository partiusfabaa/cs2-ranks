...
public class Ranks : BasePlugin
{
    ...
    public override void Load(bool hotReload)
    {
        ...
        AddTimer(1.0f, () =>
        {
            ...
            foreach (var player in Utilities.GetPlayers().Where(player => player.IsValid))
            {
                ...
                player.Clan = $"[{GetLevelFromExperience(user.value).Name}]";
            }
        }, TimerFlags.REPEAT);
        AddTimer(300.0f, () =>
        {
    ...
    private async Task ShowTopPlayers(CCSPlayerController controller)
    {
        ...
        SELECT DISTINCT * FROM `lvl_base` ORDER BY `value` DESC LIMIT 10;";
            ...
            {
                var level = GetLevelFromExperience(player.value);
                ...
                Server.NextFrame(() =>
                {
                    ...
                    controller.PrintToChat(
                        $"{rank ++}. {ChatColors.Blue}{player.name} \x01[{ChatColors.Olive}{level.Name}\x01] -\x06 Experience: {ChatColors.Blue}{player.value}");
                });
            }
        }
        catch (Exception e)
