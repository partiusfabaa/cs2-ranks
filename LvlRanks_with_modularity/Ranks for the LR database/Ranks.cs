...
public class Ranks : BasePlugin, ICorePlugin
{
    ...
    public void UpdateUserStatsLocal(CCSPlayerController? player, string msg = "",
        int exp = 0, bool increase = true, int kills = 0, int death = 0, int assist = 0,
        int shoots = 0, int hits = 0, int headshots = 0, int roundwin = 0, int roundlose = 0, bool forced = false)
    {
        if (!forced)
        {
            if (_config.MinPlayers > PlayersCount() ||
        ...
        if (increase)
        {
            if (!forced)
            {
                if (_api.PlayerGainedExperience != null) exp = _api.PlayerGainedExperience.Invoke(player, exp);
            }
        ...
        else
        {
            if (!forced)
            {
                if (_api.PlayerLostExperience != null) exp = _api.PlayerLostExperience.Invoke(player, exp);
            }
    ...
    {
        _api = new LvlCoreApi(this, _users);
        apiRegisterer.Register<ILvlCoreApi>(_api);
    }
}

public class LvlCoreApi : ILvlCoreApi
{
    private readonly Ranks _ranks;
    private readonly ConcurrentDictionary<ulong, User> _users;

    public LvlCoreApi(Ranks ranks, ConcurrentDictionary<ulong, User> users)
    {
        _ranks = ranks;
        _users = users;
    }
    ...
    {
        _ranks.UpdateUserStatsLocal(player, exp: exp, forced: true);
    }

    public void SetExperiencePlayer(CCSPlayerController player, int exp)
    {
        if (_users.TryGetValue(player.SteamID, out var user)) user.value = exp;
    }
